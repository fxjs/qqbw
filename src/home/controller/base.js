...
export default class extends think.controller.base {
    ...
    async __before() {
        ...
        // 获取导航栏
        let webNavs = await this.model("item").order("id").select();
        this.assign("webNavs", webNavs);
        console.log("FX->", webNavs);
        ...
        //获取图文推荐列表
        let picrecomList = await this.model("article").where({
            topicrecom: 1,
            ispublished: 1
        }).limit(5).select();
        ...
        //获取站长推荐列表
        let torecomList = await this.model("article").where({
            torecom: 1,
            ispublished: 1
        }).limit(5).select();
        ...
        //获取点击排行列表
        let popularList = await this.model("article").where({
            ispublished: 1
        }).order("view DESC").limit(5).select();
